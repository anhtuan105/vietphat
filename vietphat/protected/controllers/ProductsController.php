...
class ProductsController extends Controller
{
	...
	{
		$cate = Categorys::model()->find('id=:id',array(':id'=>$this->loadModel($id)->category));
		$model = $this->loadModel($id);
		// if(isset($_POST['ajax'])) {
			
		// }else 
		// if (!isset(Yii::app()->session["idclient"]))
		// {
			
		// }
		if (!isset(Yii::app()->session["view_$id"]))
		{
			$viewCount_tmp = $model->viewCount + 1;
			$model->viewCount = $viewCount_tmp;
			$connection = Yii::app()->db;
			if($connection) {
			   $sql = "update products set viewCount='$viewCount_tmp' where id=$model->id";
			   $command = $connection->createCommand($sql)->execute();
			}
			Yii::app()->session["view_$id"] = "$id";
		} 
		else
		{
			Yii::app()->session["view_$id"] = "xx_$id";
		}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Products']))
        {
            ...
            //dropdownlist to choose category when user create a new product
            ...
            if($model->save())
            {
	        	...
	        	if($uploadedFile) {
	                $uploadedFile->saveAs(Yii::app()->basePath.'/../images/Product/'.$fileName);  // image will upload to rootDirectory/images/
	        	}
	...
	public function insertData($tableName,$value)
	{
		$connection = Yii::app()->db;
		if($connection) {
		   $sql = "insert into $tableName(product_id) values('$value')";
		   $command = $connection->createCommand($sql)->execute();
		}
	}
	/**
